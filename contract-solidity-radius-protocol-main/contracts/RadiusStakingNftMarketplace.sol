...
contract RadiusStakingNftMarketplace is Ownable, ReentrancyGuard {
    ...
    /// @notice used to update the Auction duration only by owner
    /// @param _auctionDurationIncrease the auction duration
    ...
    /// @notice used to update minimum token balance only by owner
    /// @param _minTokenBalance the minimum token balance amount
    ...
    /// @notice used to withdraw the royalty
    /// @param _address the address to withdraw the royalty to
