...
interface ILdfBondParking {
    function transfer(address _user, uint256 _amount) external;
}

contract ldfTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVEMANAGER, RESERVETOKEN, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER }

    address public immutable LDF;
    address public immutable LDFBondParking;
    ...
    constructor (
        ...
        isLiquidityToken[ _LDFETH ] = true;
        liquidityTokens.push( _LDFETH );

        isLiquidityToken[ _LDFUSDC ] = true;
        liquidityTokens.push( _LDFUSDC );

        LDFBondParking = _LDFBondParking;

        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for LDF
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        if ( isReserveToken[ _token ] ) {
            ...
            require( isLiquidityDepositor[ msg.sender ], "LP Not approved" );
        }

        uint value = valueOf( _token, _amount );
        // transfer LDF needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        ILdfBondParking(LDFBondParking).transfer(msg.sender, send_);
    ...
    function manage( address _token, uint _amount ) external {
        ...
        if( isLiquidityToken[ _token ] ) {
            ...
            balance = IUniswapV2Pair(_token).balanceOf(address(this)); // checking liquidity token balance of the token in this contract
        } else {
            require( isReserveManager[ msg.sender ], "Not approved" );
            balance = IERC20(_token).balanceOf(address(this)); // checking reserve token balance of the token in this contract
        }

        uint value = valueOf( _token, _amount );
        require( value <= totalReserves, "Insufficient reserves" );
    ...
    /**
        @notice returns LDF valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match LDF decimals
            value_ = _amount.mul( 10 ** IERC20( LDF ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.RESERVEMANAGER ) { // 1
        ...
        } else if ( _managing == MANAGING.LIQUIDITYMANAGER ) { // 5
    ...
    function toggle( MANAGING _managing, address _address, address _calculator ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.RESERVEMANAGER ) { // 2
        ...
        } else if ( _managing == MANAGING.RESERVETOKEN ) { // 3
